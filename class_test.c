...
// obtain LLR of a node by recursive calls
void getLLR(node *v)
{
    if (v->lDone == 1) {    // already computed
        return;
    }
    getLLR(v->cU);
    getLLR(v->cL);
    if (v->leftP == 1) {    // it is the upperleft
