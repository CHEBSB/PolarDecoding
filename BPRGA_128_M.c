...
void BPRDEGA()
{
    ...
    printf("iterMax = %d\n", (int)(iterMax - 6 * bSNR_dB));
    for (iter = 0; iter < (iterMax - 6 * bSNR_dB); iter++) {
        // R propag
        for (i = 0; i < n; i++) 
            for (j = 0; j < N; j++) {
        ...
        // for the rest: need propagation
            for (i = 1; i <= n; i++) {
                ...
                for (j = 0; j < N; j++)
                    V[i][j]->W = log(2.0 / erfc(
                    sqrt(V[i][j]->l + V[i][j]->r) / 2.0) - 1);
                ...
                for (j = 0; j < K; j++) {
                    ...
                    V[0][bRev[I[j]]]->p = 0.5 * (1 - tanh(tempL / 2));
                }
                // compute E[i], the approx BLER
                E[i] = 0;
                for (j = 0; j < K; j++) 
