...
printf("iterMax = %d\n", iterMax);
for (bSNR_dB = 2.5; bSNR_dB <= 2.5; bSNR_dB += 0.5) {
    ...
    for (iter = 0; iter < iterMax; iter++) {
        ...
        if (iter >= 5 && iter <= 9) {
            ...
            for (i = 0; i <= n; i++) {
                // initialize u
                for (j = 0; j < N; j++)     
                    V[i][j]->u[i] = V[i][j]->l + V[i][j]->r;
                // propagate LLR until reach leftmost
                for (k = i; k > 0; k--) 
                    for (j = 0; j < N; j++) {
                        // only start from upperRight
                        if (V[k][j]->rightP == 1) {
                            // lower path
                            V[k][j]->pL->u[i] = V[k][j]->pL->cL->u[i];
                            // upper path
                            tempL = phi(V[k][j]->u[i]);
                            tempU = phi(V[k][j]->pL->cL->u[i]); 
                            V[k][j]->pU->u[i] = phi_inv(
                                tempL + tempU - tempL * tempU);
                        }
                    }
                // compute E[i], the approx BLER
                E[i] = 0;
                for (j = 0; j < K; j++) 
                    E[i] += erfc(sqrt(V[0][I[j]]->u[i]) / 2.0);
                E[i] = E[i] * 0.5;
                // printf("E%d = %lf\t", i, E[i]);
                printf("%lf\t", E[i]);
            }
